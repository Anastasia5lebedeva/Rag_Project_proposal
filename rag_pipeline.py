...
def llm_generate_kp(client_text, context_md):
    ...
    system = (
        ...
        "Задача — составить <ПРОДАВАЕМОЕ> коммерческое предложение на русском языке "
        "от лица ООО «КОРТЕКС», в стиле официальных КП.\n"
        "Источники: работай ТОЛЬКО с данными из CONTEXT. "
        "Все факты подтверждай ссылкой [CTX:DOC#order]. "
        "Не выдумывай. Если информации не хватает — напиши «предварительно».\n"
        "Опыт: используй прошлые проекты компании. Всегда выводи именно названия проектов "
        "из section_title/title_project (не используй в ответе  DOC-коды).\n"
...
# ЗАДАНИЕ
Составь ПОЛНОЦЕННОЕ коммерческое предложение ООО «КОРТЕКС» в структуре, аналогичной эталонному КП:

1. **Титульный блок** — ООО «КОРТЕКС», контакты, «КОММЕРЧЕСКОЕ ПРЕДЛОЖЕНИЕ».
2. **Наименование проекта** — по сути запроса.
3. **Цель проекта** — кратко, со ссылками [CTX].
4. **Основные задачи/система** — перечень функций, возможностей, модулей.
5. **Основные возможности системы** — развернутый список (модули, интерфейсы, интеграции).
6. **Результаты тестирования/стендирования** — если есть данные из CONTEXT; иначе «предварительно».
7. **Разработка и развертывание** — технологический стек, этапы, сроки, описание внедрения.
8. **Этапы разработки/внедрения** — таблица «Этап | Длительность | Комментарии».
9. **Стоимость** — таблица «Этап | Стоимость»; если нет данных — «предварительно».
10. **Варианты развертывания** — On-premise / SaaS, плюсы/минусы.
11. **Поставка оборудования** — если есть в CONTEXT; иначе «предварительно».
12. **Стоимость SaaS/облака** — если есть; иначе «предварительно».
13. **ОПЫТ РЕАЛИЗАЦИИ АНАЛОГИЧНЫХ ПРОЕКТОВ** — отдельный раздел с минимум 2–3 проектами из CONTEXT. 
    Таблица: «Название проекта  Масштаб | Роль ООО «КОРТЕКС» | Результаты | [CTX]».
14. **Дополнительные услуги** — интеграции, формирование КП, поддержка.
15. **Вывод/следующие шаги** — призыв к диалогу, уточнению требований.
